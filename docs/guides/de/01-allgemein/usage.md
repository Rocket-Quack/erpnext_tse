...
- Beim Submit eines **POS Closing Entry** wird die DSFinV-K-Erzeugung nur vorbereitet.
- Das eigentliche **DSFinV-K Cash Point Closing** wird erst erzeugt, wenn der `POS Closing Entry` fachlich erfolgreich auf `Submitted` steht.
- Steht das Closing zunaechst auf `Queued`, ist das noch kein Fehler.
- Der Status des Cash Point Closing sollte nach Abschluss auf **COMPLETED** stehen.
...
- Falsche oder doppelte Closings werden nicht hart geloescht, sondern kontrolliert auf `DELETED` gesetzt.

## Monatsende

Mindestens einmal pro Monat sollte geprueft werden:

- `TSE Transaction` ohne ungeklaerte `ACTIVE`, `ERROR` oder `ORPHANED`
- `DSFinV-K Cash Point Closing` ohne ungeklaerte `ERROR`
- `DSFinV-K Cash Point Closing` mit `DELETED` nur mit dokumentierter Begruendung
- `DSFinV-K Export` ohne haengende `WORKING` Eintraege
...
- `docs/guides/de/01-allgemein/troubleshooting.md`
