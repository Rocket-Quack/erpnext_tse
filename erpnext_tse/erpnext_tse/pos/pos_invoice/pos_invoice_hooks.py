...
def ensure_tse_transaction_present_and_finished(doc, method):
    ...
    if (
        getattr(tse_doc, "transaction_status", None)
        and tse_doc.transaction_status != "FINISHED"
    ):
        frappe.throw(
            _(
                "The attached TSE Transaction is not yet finished (Transaction status: {0})."
...
def show_tse_signing_success_toast(doc, method=None):
    ...
    if status == "FINISHED":
        frappe.msgprint(
            _(
                "<b>TSE-Signierung erfolgreich abgeschlossen</b><br>TSE-Transaktion: {0}"
            ).format(tse_doc.name),
            alert=True,
            indicator="green",
        )
