...
def _update_existing_from_remote(
    doc: TSESecurityDevice, remote: dict[str, Any]
) -> None:
    ...
    if remote.get("admin_puk") and not doc.get_password(
        "admin_puk", raise_exception=False
    ):
...
def _create_missing_from_remote(remote: dict[str, Any]) -> TSESecurityDevice:
    ...
    provider_status = (
        _normalize_status(remote.get("state") or remote.get("status")) or "ERROR"
    )

    # Falls bereits eine TSS mit gleicher tss_id existiert, nur aktualisieren
    existing_name = frappe.db.get_value(
        "TSE Security Device", {"tss_id": tss_id_clean}, "name"
    )
    if existing_name:
            ...
            if not remote:
                if doc.tss_status != "ORPHANED":
                    ...
                    _log_event(
                        ...
                        message=_(
                            "Marked as ORPHANED because TSS was not found at provider."
                        ),
                    )
                    _normalize_datetime_fields(doc)
                    doc.save(ignore_permissions=True)
