...
def _update_existing_from_remote(
    doc: TSEClient, remote: dict[str, Any], tss_docname: str | None
) -> bool:
    ...
    meta_pos_profile = _resolve_link(
        "POS Profile", _meta_value(meta, "pos_profile", "pos_profile_name")
    )
...
def _create_missing_from_remote(
    remote: dict[str, Any], tss_docname: str
) -> TSEClient | None:
    remote_id = _get_remote_id(remote)
    if not remote_id:
        return None
...
def run_recovery_sync(user: str | None = None, **kwargs):
    try:
        ...
        devices = frappe.get_all(
            "TSE Security Device", fields=["name", "tss_id", "tss_status"]
        )
        ...
        local_by_id = {
            _canonical_id(row.client_id): row for row in local_rows if row.client_id
        }
        local_by_name = {row.name: row for row in local_rows}
        local_by_client_name = {
            _canonical_text(row.client_name): row
            for row in local_rows
            if row.client_name
        }
        local_by_pos_profile = {
            _canonical_text(row.pos_profile): row
            for row in local_rows
            if row.pos_profile
        }
        ...
        for remote in remote_items:
            ...
            if not candidate:
                docname = _canonical_text(
                    _meta_value(meta, "tse_client_docname", "docname")
                )
                if docname:
                    candidate = local_by_name.get(docname)

            if not candidate:
                client_name = _canonical_text(
                    _meta_value(meta, "tse_client_name", "client_name", "name")
                )
                if client_name:
                    candidate = local_by_client_name.get(client_name)

            if not candidate:
                pos_profile = _canonical_text(
                    _meta_value(meta, "pos_profile", "pos_profile_name")
                )
                if pos_profile:
        ...
        frappe.db.rollback()
        frappe.log_error(frappe.get_traceback(), "TSE Client Recovery Sync failed")
        raise
