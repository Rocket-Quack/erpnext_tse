...
class TestTSEClientRecovery(FrappeTestCase):
    ...
    def test_recovery_sync_updates_creates_and_orphans(self):
        ...
        existing = self._create_client(
            "Local Client 1", device.name, "client-1", "DRAFT"
        )
        orphan = self._create_client(
            "Local Orphan", device.name, "client-missing", "REGISTERED"
        )
        ...
        created_name = frappe.db.get_value(
            "TSE Client", {"client_id": "client-2"}, "name"
        )
