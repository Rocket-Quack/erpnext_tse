...
(() => {
    ...
    const RECOVERY_EVENT = "tse_client_recovery_done";
    const RECOVERY_METHOD =
        "erpnext_tse.erpnext_tse.doctype.tse_client.recovery.enqueue_recovery_sync";
    ...
    async function getRecoveryEnabled() {
        try {
            const v = await frappe.db.get_single_value(
                SETTINGS_DOCTYPE,
                SETTINGS_FLAG_FIELD
            );
            return toInt(v) === 1;
    ...
    function indicatorFor(doc) {
        const status = doc.client_status || "DRAFT";
        const conf = INDICATORS[status] || { color: "gray" };
        return [__(status), conf.color, `client_status,=,${status}`];
    }
