...
def _create_missing_from_remote(
    ...
    *,
    ...
    existing_name = frappe.db.get_value(
        "TSE Transaction", {"transaction_id": remote_id}, "name"
    )
    if existing_name:
...
def run_recovery_sync(user: str | None = None, **kwargs):
    try:
        ...
        client_by_id = {
            _canonical_id(row.client_id): row for row in clients if row.client_id
        }
        ...
        local_by_id = {
            _canonical_id(row.transaction_id): row
